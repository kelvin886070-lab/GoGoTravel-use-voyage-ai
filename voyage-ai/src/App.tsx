...
const App: React.FC = () => {
  ...
  return (
    // 修正: 確保全螢幕覆蓋，包含 safe-area (pt-safe-top)
    <div className="h-[100dvh] w-full font-sans text-[#1D1D1B] bg-[#E4E2DD] overflow-hidden fixed inset-0 pt-safe-top" style={{ backgroundImage: bgImage ? `url(${bgImage})` : 'none', backgroundSize: 'cover', backgroundPosition: 'center' }}>
      ...
      <main className="max-w-md mx-auto h-full relative shadow-2xl overflow-hidden z-10 bg-[#E4E2DD]/80 backdrop-blur-md flex flex-col">
        ...
        {/* 修正: Tab Bar 高度壓縮、Padding 調整，打造 Instagram 般的緊湊感 */}
        <div className="flex-shrink-0 z-50 relative w-full bg-white/95 backdrop-blur-xl border-t border-white/50 shadow-[0_-5px_20px_rgba(0,0,0,0.02)]">
            <div className="flex justify-between items-center pb-safe pt-2 px-6 h-[calc(55px+env(safe-area-inset-bottom))]">
...
// 修正: 圖示縮小、文字縮小 (text-[9px])、間距變緊 (gap-0.5)
const TabButton: React.FC<{ active: boolean, onClick: () => void, icon: React.ReactNode, label: string }> = ({ active, onClick, icon, label }) => (
  <button onClick={onClick} className={`flex flex-col items-center gap-0.5 transition-all duration-300 w-16 ${active ? 'text-[#45846D] scale-105' : 'text-gray-400 hover:text-gray-600'}`}>
