...
const WeatherWidget: React.FC = () => {
    ...
    const fetchWeather = async () => { const currentLocation = locations[idx];
    const cacheKey = `Kelvin Trip_weather_cache_${currentLocation}`; try { const cached = localStorage.getItem(cacheKey);
    if (cached) { const { data, timestamp } = JSON.parse(cached);
    if (Date.now() - timestamp < 30 * 60 * 1000) { setData(data); setLoading(false); return; } } } catch(e) {} setLoading(true);
    try { const res = await getWeatherForecast(currentLocation); if(res) { setData(res); localStorage.setItem(cacheKey, JSON.stringify({ data: res, timestamp: Date.now() }));
    ...
    const handleDelete = (e: React.MouseEvent) => { e.stopPropagation(); if(locations.length > 1) { const newLocs = locations.filter((_, i) => i !== idx);
    setLocations(newLocs); setIdx(0); } };
    const getWeatherIcon = (condition: string = '') => { if(condition.includes('雨')) return <CloudRain className="w-8 h-8 text-blue-200" />;
    if(condition.includes('雲') || condition.includes('陰')) return <Cloud className="w-8 h-8 text-gray-200" />; if(condition.includes('晴')) return <Sun className="w-8 h-8 text-yellow-300" />;
...
const FlightCard = ({ type, code, setCode, destination, origin = 'TPE', flightInfo, loading }: any) => {
    ...
    return (
        <div className={`${bgColor} rounded-3xl p-6 shadow-lg relative overflow-hidden text-white group transition-transform active:scale-[0.98]`}>
            ...
            <div className="absolute -right-3 top-1/2 w-6 h-6 bg-[#F5F5F4] rounded-full z-10" />
            ...
            <div className="flex justify-between items-start mb-8">
                ...
                <div className="bg-white/10 p-2 rounded-xl backdrop-blur-md border border-white/10 text-center min-w-[70px]">
                    <span className="text-[10px] font-bold tracking-tight block text-white/60">TIME</span>
                    {loading ?
                    (
                         <div className="flex justify-center mt-1"><Loader2 className="w-4 h-4 animate-spin text-white" /></div>
                    ) : (
                         <span className="text-base font-bold">{displayTime || '--:--'}</span>
                    )}
                </div>
            </div>
...
const TrainCard = ({ label, info, setInfo }: { label: string, info: any, setInfo: (i: any) => void }) => (
    <div className="bg-white border-l-[6px] border-l-orange-500 border-y border-r border-gray-100 rounded-r-2xl rounded-l-md p-5 shadow-sm relative group hover:shadow-md transition-all">
        <div className="flex justify-between items-center mb-4"><span className="text-[10px] font-bold text-gray-400 uppercase tracking-widest flex items-center gap-2"><Train className="w-4 h-4 text-orange-500" /> {label}</span><span className="bg-orange-100 text-orange-600 text-[10px] font-bold px-2 py-1 rounded">RAIL PASS</span></div>
        ...
        <div className="flex gap-2"><div className="flex-[2]"><input type="text" placeholder="車種 (新幹線/高鐵)" value={info.type} onChange={(e) => setInfo({...info, type: e.target.value})} className="w-full bg-gray-50 rounded-xl px-3 py-2.5 text-xs font-bold text-gray-700 outline-none focus:ring-2 focus:ring-orange-200/50 transition-all"/></div><div className="flex-[1]"><input type="text" placeholder="車次" value={info.number} onChange={(e) => setInfo({...info, 
        number: e.target.value})} className="w-full bg-gray-50 rounded-xl px-3 py-2.5 text-xs font-bold text-gray-700 outline-none text-center focus:ring-2 focus:ring-orange-200/50 transition-all font-mono"/></div></div>
    </div>
);
...
const EditTripModal: React.FC<{ trip: Trip, onClose: () => void, onUpdate: (t: Trip) => void }> = ({ trip, onClose, onUpdate }) => {
    ...
    finalEndDateObj.setDate(startDateObj.getDate() + (daysCount - 1)); let newDays = [...trip.days]; if (daysCount > trip.days.length) { for (let i = trip.days.length + 1; i <= daysCount; i++) { newDays.push({ day: i, activities: [] });
    } } else if (daysCount < trip.days.length) { newDays = newDays.slice(0, daysCount);
    } onUpdate({ ...trip, destination: dest, startDate: start, endDate: finalEndDateObj.toISOString().split('T')[0], days: newDays }); };
    return (<div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center sm:p-4"><div className="absolute inset-0 bg-[#1D1D1B]/40 backdrop-blur-sm" onClick={onClose} /><div className="bg-white w-full max-w-sm sm:rounded-[32px] rounded-t-[32px] p-6 relative z-10 shadow-2xl animate-in slide-in-from-bottom"><div className="flex justify-between items-center mb-6"><h3 className="text-xl font-bold text-[#1D1D1B]">編輯行程資訊</h3><button onClick={onClose}><X className="w-5 h-5" /></button></div><div className="space-y-4"><div><label className="block text-xs font-bold text-gray-400 mb-1 ml-1 uppercase">目的地</label><IOSInput value={dest} onChange={e => setDest(e.target.value)} /></div><div><label className="block text-xs font-bold text-gray-400 mb-1 ml-1 uppercase">開始日期</label><input type="date" value={start} onChange={e => setStart(e.target.value)} className="w-full bg-[#F5F5F4] p-4 rounded-2xl outline-none" /></div><div><label className="block text-xs font-bold text-gray-400 mb-1 ml-1 uppercase">總天數</label><IOSInput type="number" min={1} max={30} value={daysCount} onChange={e => setDaysCount(Number(e.target.value))} />{daysCount < trip.days.length && <p className="text-red-500 text-xs mt-1 ml-1">注意：減少天數將會刪除末尾的行程安排。</p>}</div><IOSButton fullWidth onClick={handleSave}>儲存變更</IOSButton></div></div></div>);
};
...
const CreateTripModal: React.FC<{ onClose: () => void, onAddTrip: (t: Trip) => void }> = ({ onClose, onAddTrip }) => {
    ...
    const buildPrompt = () => {
        ...
        - 步調：${paceMap[pace]}
        - 風格：${vibeMap[vibe]}
        - 預算：${budgetMap[budgetLevel]} ${customBudget ?
        `(${customBudget})` : ''}
        - 興趣細項：${interestsWithDetails ||
        '無特別指定'}
    ...
    const handleCreate = async () => {
        ...
        try {
            ...
            if (transportMode === 'flight') {
                transportInfo = { 
                    inbound: flightIn ?
                    `Flight ${flightIn} (${flightInInfo?.depTime || 'TBA'} - ${flightInInfo?.arrTime || 'TBA'})` : undefined, 
                    outbound: flightOut ?
                    `Flight ${flightOut} (${flightOutInfo?.depTime || 'TBA'})` : undefined 
                };
            } else if (transportMode === 'train') {
                transportInfo = { inbound: trainIn.number ?
                `${trainIn.type} ${trainIn.number} from ${trainIn.origin} to ${trainIn.dest}` : undefined, outbound: trainOut.number ? `${trainOut.type} ${trainOut.number}` : undefined };
            }
            ...
            const processGeneratedItinerary = (days: TripDay[]): TripDay[] => {
                return days.map(day => {
                    ...
                    const activities = day.activities.map(act => {
                        if (!act.time 
                        || !/^\d{2}:\d{2}$/.test(act.time)) act.time = nextStartTime;
    ...
    const handleManualCreate = () => {
        ...
        const newTrip: Trip = { id: Date.now().toString(), destination: destination || '未命名行程', startDate: startDate, endDate: endDateObj.toISOString().split('T')[0], coverImage: coverImage ||
        `https://picsum.photos/800/600?random=${Date.now()}`, days: emptyDays, isDeleted: false, currency: currency };
    ...
    return (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
            ...
            <div className="bg-white w-full max-w-md rounded-[32px] overflow-hidden shadow-2xl relative z-10 flex flex-col max-h-[90vh]">
                {/* Header */}
                <div className="pt-6 px-6 pb-2 border-b border-gray-100 bg-white sticky top-0 
                z-20">
                    <div className="flex justify-between items-center mb-4">
                        <button onClick={step === 1 ? onClose : () => setStep(s => s - 1)} className="text-gray-400 hover:text-gray-600">
                            {step === 1 ? <X className="w-6 h-6" /> : 
                    ...
                    <div className="flex gap-2 mb-2">
                        {[1, 2, 
                        3, 4].map(i => (<div key={i} className={`h-1 rounded-full flex-1 transition-all duration-500 ${i <= step ?
                        'bg-[#45846D]' : 'bg-gray-100'}`} />))}
                    </div>
                </div>
                    ...
                    1 && (
                        <div className="space-y-6 animate-in slide-in-from-right-4 duration-300">
                            <div className="relative w-full h-48 rounded-2xl overflow-hidden bg-[#F5F5F4] group">
                                {coverImage 
                                ? (<img src={coverImage} alt="Cover" className="w-full h-full object-cover" />) : (<div className="w-full h-full flex flex-col items-center justify-center text-gray-400"><ImageIcon className="w-8 h-8 mb-2 opacity-50" /><span className="text-xs font-medium">設定封面 (選填)</span></div>)}
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    ...
                                    <div><label 
                                    className="block text-xs font-bold text-gray-400 mb-1 ml-1 uppercase">天數</label><IOSInput type="number" min={1} max={14} value={days} placeholder="輸入天數" onChange={(e) => { const val = e.target.value;
                                    setDays(val === '' ? '' : parseInt(val, 10)); }} /></div>
                                </div>
                            </div>
                        </div>
                    )}
                    {step === 2 && (
                        <div className="space-y-6 animate-in slide-in-from-right-4 duration-300">
                            <div className="bg-[#F5F5F4] p-1.5 rounded-2xl flex mb-6">
                                ...
                                <button onClick={() => setTransportMode('train')} className={`flex-1 flex items-center justify-center gap-2 py-3 text-xs font-bold 
                                rounded-xl transition-all ${transportMode === 'train' ? 'bg-white shadow-sm text-[#1D1D1B]' : 'text-gray-400 hover:text-gray-600'}`}><Train className="w-4 h-4"/> 列車</button>
                                <button onClick={() => setTransportMode('time')} className={`flex-1 flex items-center justify-center gap-2 py-3 text-xs font-bold rounded-xl transition-all ${transportMode === 'time' ?
                                'bg-white shadow-sm text-[#1D1D1B]' : 'text-gray-400 hover:text-gray-600'}`}><Clock className="w-4 h-4"/> 手動</button>
                            </div>
                        ...
                        <div className="space-y-6 animate-in slide-in-from-right-4 duration-300">
                            <div>
                                ...
                                <div className="grid grid-cols-4 gap-2">
                                    ...
                                    <OptionCard selected={companion === 
                                    'friends'} onClick={() => setCompanion('friends')} icon={<Users />} label="朋友" />
                            ...
                            <div>
                                ...
                                text-gray-400 mb-2 ml-1 uppercase">預算等級</label>
                                <div className="flex gap-2 mb-3">
                                    {[{id:'cheap',l:'經濟 $'}, {id:'standard',l:'標準 $$'}, {id:'luxury',l:'豪華 $$$'}].map(opt => (
                                     <button key={opt.id} onClick={() => setBudgetLevel(opt.id)} className={`flex-1 py-3 rounded-2xl font-bold text-sm border transition-all ${budgetLevel === opt.id ?
                                    'bg-[#1D1D1B] text-white border-[#1D1D1B]' : 'bg-white text-gray-500 border-gray-100 hover:bg-gray-50'}`}>{opt.l}</button>
                                    ))}
                                </div>
                                <div className="flex gap-2">
                                    <div className="relative min-w-fit">
                                        ...
                                        <select value={currency} onChange={(e) => setCurrency(e.target.value)} className="w-full bg-[#F5F5F4] border-none rounded-xl py-3 pl-9 pr-8 text-sm outline-none focus:ring-2 focus:ring-[#45846D]/50 appearance-none font-bold">{CURRENCIES.map(c => (<option key={c.code} value={c.code}>{c.code} {c.label}</option>))}</select>
                                    </div>
                ...
                <div className="p-6 border-t border-gray-100 bg-white sticky bottom-0 z-30">
                    ...
                    (
                        <div className="flex flex-col gap-3">
                            ...
                            {step === 1 && <button onClick={handleManualCreate} className="text-gray-400 text-xs font-medium 
                            py-2 hover:text-gray-600 transition-colors">跳過 AI，手動建立空白行程</button>}
                        </div>
                    ) : (
                        <div className="flex gap-3">
                            <button onClick={handleManualCreate} className="flex-1 py-3 text-gray-500 font-bold text-sm bg-white border border-gray-200 rounded-full hover:bg-gray-50">手動建立</button>
...
const ImportTripModal: React.FC<{ onClose: () => void, onImportTrip: (t: Trip) => void }> = ({ onClose, onImportTrip }) => {
    ...
    const handleImport = () => { try { if (!code.trim()) return; const jsonString = decodeURIComponent(escape(atob(code.trim())));
    const tripData = JSON.parse(jsonString); if (tripData && tripData.destination && tripData.days) { onImportTrip(tripData); onClose(); } else { setError('無效的行程連結');
...
const ProfileModal: React.FC<{ user: User, tripCount: number, onClose: () => void, onLogout: () => void }> = ({ user, tripCount, onClose, onLogout }) => {
    ...
    const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => { try { setUploading(true); if (!e.target.files || e.target.files.length === 0) return;
    const file = e.target.files[0]; if (file.size > 2 * 1024 * 1024) { alert('圖片太大了！請上傳小於 2MB 的照片。'); return;
    } const fileExt = file.name.split('.').pop(); const fileName = `avatar_${Date.now()}.${fileExt}`; const filePath = `${user.id}/${fileName}`;
...
export const TripsView: React.FC<TripsViewProps> = ({ trips, user, onLogout, onAddTrip, onImportTrip, onSelectTrip, onDeleteTrip, onReorderTrips, onUpdateTrip }) => {
  ...
  return (
    <div className="h-full flex flex-col w-full bg-transparent">
      ...
      <div className="flex-1 min-h-0 overflow-y-auto px-5 pt-4 space-y-6 pb-24 w-full scroll-smooth no-scrollbar">
        ...
        <div className="bg-white/50 p-1 rounded-2xl flex relative mb-2 shadow-inner">
            <button 
                ...
                className={`flex-1 py-2 text-sm font-bold rounded-xl transition-all duration-200 ease-out flex items-center justify-center ${activeTab === 'upcoming' ?
                'bg-white text-[#1D1D1B] shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
             >
                即將出發
             </button>
             <button 
                ...
                font-bold rounded-xl transition-all duration-200 ease-out flex items-center justify-center ${activeTab === 'past' ?
                'bg-white text-[#1D1D1B] shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
             >
                精彩回憶
             </button>
        ...
        <div className="mt-2">
            <DragDropContext onDragEnd={onDragEnd}>
                ...
                (
                    <div className="text-center py-12 opacity-40 bg-[#1D1D1B]/5 rounded-3xl border border-dashed border-[#1D1D1B]/20 mx-1">
                        {activeTab === 'upcoming' ? (
                             <><MapPin className="w-10 h-10 mx-auto mb-3 text-gray-500" /><p className="text-sm font-bold text-gray-600">尚無計畫，點擊右上角 + 開始規劃</p></>
                        ) : (
                             <><History className="w-10 h-10 mx-auto mb-3 text-gray-500" /><p className="text-sm font-bold text-gray-600">還沒有結束的旅程，趕快出發吧！</p></>
                        )}
                    </div>
                ) : (
      ...
      {isImporting && <ImportTripModal 
        onClose={() => setIsImporting(false)} onImportTrip={onImportTrip} />}
