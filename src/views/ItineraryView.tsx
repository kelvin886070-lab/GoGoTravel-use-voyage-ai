...
const Tag: React.FC<{ type: string }> = ({ type }) => {
    const config: Record<string, { color: string, label: string, icon: any }> = {
        sightseeing: { color: 'bg-blue-100 text-blue-600', label: '景點', icon: Camera },
        food: { color: 'bg-orange-100 text-orange-600', label: '美食', icon: Utensils },
        transport: { color: 'bg-gray-100 text-gray-600', label: '交通', icon: Bus },
        flight: { color: 'bg-purple-100 text-purple-600', label: '航班', icon: Plane },
        hotel: { color: 'bg-indigo-100 text-indigo-600', label: '住宿', icon: Bed },
        cafe: { color: 'bg-amber-100 text-amber-700', label: '咖啡廳', icon: Utensils },
        shopping: { color: 'bg-pink-100 text-pink-600', label: '購物', icon: TagIcon },
        relax: { color: 'bg-emerald-100 text-emerald-600', label: '放鬆', icon: TagIcon },
        bar: { color: 'bg-violet-100 text-violet-600', label: '酒吧', icon: TagIcon },
        culture: { color: 'bg-rose-100 text-rose-600', label: '文化', icon: TagIcon },
                                ...
                                <div className="bg-white w-full max-w-xs rounded-2xl p-4 relative z-10 shadow-2xl animate-in zoom-in-95">
                                    ...
                                    <div className="space-y-2 max-h-[60vh] overflow-y-auto p-1">
                                        {Object.keys(CURRENCY_SYMBOLS).map(cur => (
                                            <button key={cur} onClick={() => handleCurrencyChange(cur)} className={`w-full flex justify-between items-center px-4 py-3 rounded-xl text-sm font-medium transition-all active:scale-95 ${currencyCode === cur ? 'bg-ios-blue text-white shadow-md' : 'bg-gray-50 text-gray-800 hover:bg-gray-100'}`}>
                                                <span>{CURRENCY_LABELS[cur] || cur}</span><span className={`font-mono ${currencyCode === cur ? 'text-blue-100' : 'text-gray-400'}`}>{CURRENCY_SYMBOLS[cur]}</span>
                                            </button>
                                        ))}
                                    </div>
                                </div>
                                                                            ...
                                                                            <div className="flex-1 min-w-0 border-l border-gray-100 pl-3">
                                                                                ...
                                                                                <div className="flex items-center justify-between mt-1">
                                                                                    ...
                                                                                    <div className="flex items-center gap-0.5 text-xs text-gray-400 bg-gray-50 px-2 py-0.5 rounded-md hover:bg-gray-100 transition-colors cursor-text min-h-[24px]">
                                                                                        ...
                                                                                        <AutoWidthInput value={parseCost(act.cost)} onChange={(val) => handleActivityUpdate(dayIndex, index, 'cost', val)} className="text-gray-600 font-medium text-right" />
                                                                                    </div>
                                                                                </div>
